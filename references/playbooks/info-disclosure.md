...
**找到密钥后必须做「用途分析」才能定危害**（跳过这步会给出过高或过低的评级）`[2026-09 实战沉淀]`：

| 分析步骤 | 做法 |
|---|---|
| ① 这个密钥**加密什么** | 在 bundle 里 grep 密钥变量名 / 加解密函数名（如 `encryptByAES`/`decryptByAES`/`useEncryption`），**列出所有调用点** |
| ② 密文**谁能拿到** | 前端加密 → 密文在浏览器；localStorage 加密 → 密文在本地存储；接口加密 → 密文在请求里 |
| ③ 拿到密文**需不需要额外前提** | 需要 XSS？物理接触？中间人？共享设备？浏览器扩展？ |
| ④ 最坏影响是什么 | 能解密什么、能伪造什么、下游还有没有别的校验兜底 |

**实战示例（本次）**：某站前端硬编码 AES key/iv，经调用点分析发现用于两处 ——

| 用途 | 危害分析 | 结论 |
|---|---|---|
| **业务二维码加解密** | 密钥公开 → 可伪造二维码；但伪造后**仍需真实核销员扫码**（核销环节有白名单校验）→ 危害受限 | 中 |
| **localStorage 中 JWT/用户信息的加密存储** | **该加密对 XSS 场景本就无效**（攻击者可直接调用前端自带的解密函数，根本不需要密钥）；唯一理论价值在「本地存储被第三方获取」的场景，而密钥公开使该价值归零 | 低 |

> **纪律**：不要只写「密钥公开 → 可解密敏感数据」。**必须先说清密钥实际保护什么、拿到密文需要什么前提，再给危害**——否则甲方无法判断修复优先级，也会让评级显得凭空。
> 本次结论是「**加密实现缺陷**」而非「可直接利用的漏洞」，评级相应下调。
