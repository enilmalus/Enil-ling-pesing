...
8. **降级原则**：黑名单被拦 → 换大小写/双写/特殊后缀 → 白名单被拦 → **换一条代码路径**（复制 / 重命名 / 移动 / 解压等二次入口，见 §4.7）→ 解析漏洞/`.htaccess`/路径穿越 → 内容扫描被拦 → 变量函数/编码/拼接。
...
**经二次代码路径落地时**（§4.7）额外需要：⑤ 触发改名/复制的那次操作的**完整请求与响应**（用来证明"目标文件名未过白名单校验"，而非仅上传成功）；⑥ 落地文件直链访问的请求+响应（证明它落在 Web 可访问目录且被服务端解析）。
